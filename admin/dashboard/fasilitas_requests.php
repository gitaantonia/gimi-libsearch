...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['id_booking']) && isset($_POST['action'])) {
    $id_booking = $_POST['id_booking'];
    $action = $_POST['action'];
    
    if ($action === 'acc') {
        $query = "UPDATE bookings SET status_booking = 'confirmed' WHERE id_booking = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $id_booking);
        
        if ($stmt->execute()) {
            $_SESSION['msg'] = "Facility request approved successfully.";
        ...
        $query = "UPDATE bookings SET status_booking = 'cancelled' WHERE id_booking = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $id_booking);
        
        if ($stmt->execute()) {
            $_SESSION['msg'] = "Facility request rejected successfully.";
            $_SESSION['msg_type'] = "success";
        } else {
    ...
    header("Location: fasilitas_requests.php");
    exit;
}
...
if ($status_filter !== '') {
    $where_clauses[] = "b.status_booking = ?";
...
$query = "
    SELECT 
        ...
        f.nama_fasilitas
    FROM bookings b
    LEFT JOIN pengguna p ON b.id_anggota = p.id_pengguna
    LEFT JOIN fasilitas f ON b.id_fasilitas = f.id
    WHERE $where_sql
...
<title>Facility Requests - GiMi Library</title>
...
<main class="flex-1 flex flex-col overflow-y-auto bg-[#0b0d10]">
    ...
    <header class="px-8 py-6 border-b border-[#2d3139] flex justify-between items-end">
        <div>
            ...
            <h2 class="text-2xl font-semibold text-white tracking-tight">Facility Requests Management</h2>
        </div>
    </header>
    ...
    <div class="p-8 space-y-6 max-w-[1400px]">
        ...
        <form method="GET" action="fasilitas_requests.php" class="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <div class="relative w-full sm:w-auto flex-1 max-w-sm">
                ...
                <input type="text" name="search" class="w-full bg-[#15181e] border border-[#2d3139] text-gray-200 text-sm rounded-lg block pl-10 p-2.5 transition-colors focus:border-blue-500 focus:ring-1 focus:ring-blue-500" placeholder="Search for members / facility..." value="<?php echo htmlspecialchars($search); ?>">
            </div>
            ...
            <div class="w-full sm:w-auto flex gap-2">
                ...
                <a href="fasilitas_requests.php" class="bg-[#1a1d24] hover:bg-[#2d3139] text-gray-300 px-4 py-2.5 rounded-lg border border-[#2d3139] flex items-center gap-2 transition-colors text-sm font-medium">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-[#1a1d24] text-gray-400 text-xs uppercase tracking-wider border-b border-[#2d3139]">
                            ...
                            <th class="px-6 py-4 font-medium">Booking Schedule</th>
                    ...
                    <tbody class="divide-y divide-[#2d3139]">
                        ...
                        <tr class="hover:bg-[#1a1d24] transition-colors">
                            <td class="px-6 py-4 text-gray-300">#<?php echo $row['id_booking']; ?></td>
                            <td class="px-6 py-4 font-medium text-white"><?php echo htmlspecialchars($row['nama']); ?></td>
                            <td class="px-6 py-4">
                                <?php 
                                if (empty($row['id_fasilitas'])) {
                                    echo '<div class="flex items-center gap-2 text-yellow-500/90 text-xs font-medium bg-yellow-500/10 px-2 py-1 rounded w-fit border border-yellow-500/20"><i class="fa-solid fa-triangle-exclamation"></i> Facility not found</div>';
                                } else {
                                    echo '<div class="text-gray-200">'.htmlspecialchars($row['nama_fasilitas']).'</div>';
                                }
                                ?>
                            </td>
                            <td class="px-6 py-4 text-gray-400 text-sm whitespace-nowrap">
                                <div><?php echo date('d M Y', strtotime($row['tanggal'])); ?></div>
                                <div class="text-xs text-gray-500 mt-1"><?php echo date('H:i', strtotime($row['jam_mulai'])); ?> - <?php echo date('H:i', strtotime($row['jam_selesai'])); ?></div>
                            </td>
                            <td class="px-6 py-4">
                                <?php 
                                if ($row['status_booking'] == 'pending') {
                                    ...
                                    echo '<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-green-500/20 text-green-400 border border-green-500/30">Confirmed</span>';
                                } elseif ($row['status_booking'] == 'cancelled') {
                                    echo '<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-red-500/20 text-red-400 border border-red-500/30">Cancelled</span>';
                                } else {
                                    echo '<span class="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-gray-500/20 text-gray-400 border border-gray-500/30">'.htmlspecialchars(ucfirst($row['status_booking'])).'</span>';
                                }
                                ?>
                            </td>
                                ...
                                <?php if ($row['status_booking'] == 'pending'): ?>
                                    <div class="flex justify-end gap-2">
                                        <form action="" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="bg-green-600 hover:bg-green-500 text-white px-3 py-1.5 rounded-lg text-xs font-medium transition-colors flex items-center gap-1.5">
                                                <i class="fa-solid fa-check"></i> ACC
                                            </button>
                                        </form>
                                        <button onclick="openRejectModal(<?php echo $row['id_booking']; ?>)" class="bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/30 px-2.5 py-1.5 rounded-lg transition-colors"><i class="fa-solid fa-xmark"></i></button>
                                    </div>
                                <?php else: ?>
                        ...
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p>There are no facility requests yet.</p>
                                </div>
                            </td>
                        </tr>
                        <?php endif; ?>
    ...
    <div class="bg-[#15181e] border border-[#2d3139] rounded-xl shadow-2xl z-10 w-full max-w-sm mx-4 transform transition-all">
        <form action="" method="POST">
            <div class="p-6 text-center">
                <input type="hidden" name="id_booking" id="reject_id_booking">
...
<script>
    ...
    const rejectBookingId = document.getElementById('reject_id_booking');
