...
$query = "
    SELECT p.id_peminjaman, u.nama, b.judul, p.tanggal_pinjam, p.tanggal_kembali, p.status as loan_status,
           DATEDIFF(CURRENT_DATE(), p.tanggal_kembali) as days_late
    FROM peminjaman p
...
$totalActive = 0;
$totalLate = 0;
$qStats = mysqli_query($conn, "SELECT COUNT(*) as tot, SUM(CASE WHEN DATEDIFF(CURRENT_DATE(), tanggal_kembali) > 0 THEN 1 ELSE 0 END) as late FROM peminjaman WHERE status = 'dipinjam'");
...
<script>
    ...
    function calculateFine() {
        ...
        if (type === 'Terlambat') {
            ...
            if (type === 'Rusak') { fine = 50000; }
            if (type === 'Hilang') { fine = 100000; }
        }
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-1 text-xs">
                                        <div class="flex items-center gap-2">
                                            ...
                                            <span class="text-gray-400"><?php echo date('d M Y', strtotime($row['tanggal_pinjam'])); ?></span>
                                        </div>
                                        <div class="flex items-center gap-2">
                                            <span class="text-gray-500 w-10">To:</span>
                                            <span class="text-gray-300 font-medium"><?php echo date('d M Y', strtotime($row['tanggal_kembali'])); ?></span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
